 ...
 */
export const useTypeApi = ({ apiList = [], realTime, initialData, cacheStrategy, filterCount, opt = {
    limit: 1000,
    cursor: '',
    ...
    const finalLeaderboardData = useMemo(() => leaderboardData?.map(leaderboard => {
        const seenIds = new Set();
        const filteredLeaderboard = leaderboard.reduce((acc, item) => {
    ...
    const getApiPromiseList = useCallback((strategy, apis = []) => apis
        ...
        .filter((i) => Boolean(i)), [env, options]);
